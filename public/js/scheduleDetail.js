...
function check() {
    ...
    if(form.planid.value=="" || form.studioname.value=="" || form.playname.value=="" || form.date.value=="" || form.ticket.value==""){
        alert("请填写完整信息");
        return;
    }
    if(isNaN(form.ticket.value)){
        alert("生成票数不是数字");
        return;
    }
    var url = "";
    ...
    if (req) {
        ...
        var data = "type=" + form.type.value + "&planid="
            + form.planid.value + "&studioname="
            + encodeURIComponent(form.studioname.value) + "&playname="
            + encodeURIComponent(form.playname.value) + "&date=" + encodeURIComponent(form.date.value)
