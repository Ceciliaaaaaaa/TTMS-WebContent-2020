...
function searchComplete() {
    if (req.readyState == 4 && req.status == 200) {
        ...
        for(i=0; i<json.length; i++){
            ...
            row.insertCell(0).innerHTML = json[i].playid;
            row.insertCell(1).innerHTML = json[i].playname;
            row.insertCell(2).innerHTML = json[i].period;
            row.insertCell(3).innerHTML = json[i].num;
            row.insertCell(4).innerHTML = json[i].total;
        }
    }
}
