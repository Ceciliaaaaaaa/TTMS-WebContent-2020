function search(){
    ...
    if (req) {
        ...
        req.send("name="+document.getElementById("staffname").value);
    }
}
...
function searchComplete() {
    if (req.readyState == 4 && req.status == 200) {
        ...
        for(i=0; i<json.length; i++){
            ...
            var tmp = json[i].id + ",\'" + json[i].name + "\'," + json[i].email + "," + json[i].password + ",\'";
            row.insertCell(4).innerHTML = '<button type="button" class="btn btn-outline-primary update" onclick="modify('+ tmp +')">修改</button><button type="button" class="btn btn-outline-danger delete" onclick="del('+json[i].id+')">删除</button>';
        }
    }
}

function modify(a,b,c,d){
    window.location="staffDetail.html?type=modify&id="+a+"&name="+encodeURIComponent(b)+"&email="+encodeURIComponent(c)+"&password="+encodeURIComponent(d);
}
